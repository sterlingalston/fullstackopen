...
const hook = () => {
 ...
 const [ newName, setNewName ] = useState({name: '', number: ''})
  ...
  const handleNameChange = (event) =>{
  		...
  		setNewName({name: event.target.value, number: newName.phone})
  }

  const handlePhoneChange = (event) =>{
  		console.log(event.target.value)
  		setNewName({name: newName.name, number: event.target.value})
  }
  		...
  		if(persons.filter(p => p.name == newName.name).length)

  			{
  				...
  				if(window.confirm(`${newName.name} is already added to phonebook, replace the old number with the new one?`)){
  					...
  					let changedPerson = {...fndPerson,number: newName.phone}
